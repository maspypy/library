...
data:
  ...
  code: "/*\np \u3092\u30A4\u30F3\u30AF\u30EA\u30E1\u30F3\u30C8\u3057\u306A\u304C\u3089\
    ...
    \ && st.back().se <= A[i]) {\n        auto [RR, x] = POP(st);\n        if (x !=\
    \ A[i]) event_left.eb(i, R, RR, x, A[i]);\n        R = RR;\n      }\n      st.eb(R,\
    \ A[i]);\n    }\n\n    f(0, 1, -infty<T>);\n    int L = 1;\n    for (int k = len(st);\
    \ k--;) {\n      auto [R, x] = st[k];\n      f(L, R, x);\n      L = R;\n    }\n\
    \  }\n};"
  dependsOn: []
  ...
  verifiedWith: []
documentation_of: ds/range_max_change.hpp
layout: document
redirect_from:
- /library/ds/range_max_change.hpp
- /library/ds/range_max_change.hpp.html
