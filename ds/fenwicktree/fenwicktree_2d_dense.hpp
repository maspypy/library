...
struct FenwickTree_2D_Dense {
  ...
  FenwickTree_2D_Dense(int H, int W) : H(H), W(W), dat(H * W, G::id()) {}
  ...
  E prod(int lx, int rx, int ly, int ry) {
    E pos = G::id(), neg = G::id();
    while (lx < rx) {
      pos = G::op(pos, sum_x(rx, ly, ry)), rx -= rx & -rx;
    }
    while (rx < lx) {
      neg = G::op(neg, sum_x(lx, ly, ry)), lx -= lx & -lx;
    }
 ...
 private:
  ...
  void add_x(int x, int y, E val) {
    ++y;
    while (y <= W) {
      dat[idx(x, y)] = G::op(dat[idx(x, y)], val), y += y & -y;
    }
  }
  E sum_x(int x, int ly, int ry) {
    E pos = G::id(), neg = G::id();
    while (ly < ry) {
      pos = G::op(pos, dat[idx(x, ry)]), ry -= ry & -ry;
    }
    while (ry < ly) {
      neg = G::op(neg, dat[idx(x, ly)]), ly -= ly & -ly;
    }
    return G::op(pos, G::inverse(neg));
  }
