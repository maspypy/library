...
data:
  ...
  bundledCode: "#line 1 \"test/3_yukicoder/705_2.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/705\"\
    ...
    \ n - 1)] : min<int>(x + lo, hi - 1))); }\n};\n#line 5 \"test/3_yukicoder/705_2.test.cpp\"\
    \n\nint N;\nvi dp;\nvc<int> A, X, Y;\n\nstruct F {\n  using value_type = ll; //\
    \ operator() \u306E\u623B\u308A\u5024\n  int i;\n  ll operator()(int j) {\n  \
    \  if (i == -1) return infty<ll>;\n    assert(1 <= j && j <= N);\n    ll dx =\
    \ abs(A[j - 1] - X[i]);\n    ll dy = abs(Y[i]);\n    return dp[i] + dx * dx *\
    \ dx + dy * dy * dy;\n  }\n};\n\nvoid solve() {\n  read(N);\n  A.resize(N), X.resize(N),\
    \ Y.resize(N);\n  FOR(i, N) read(A[i]);\n  FOR(i, N) read(X[i]);\n  FOR(i, N)\
    \ read(Y[i]);\n\n  dp.resize(N + 1);\n  LiChao_Tree<F, false, true> LCT(1, N +\
  ...
  requiredBy: []
  timestamp: '2025-01-20 22:40:00+09:00'
