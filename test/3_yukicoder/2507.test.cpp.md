...
data:
  ...
  bundledCode: "#line 1 \"test/3_yukicoder/2507.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/2507\"\
    ...
    \ TIDAK(bool t = 1) { YA(!t); }\r\n#line 4 \"test/3_yukicoder/2507.test.cpp\"\n\
    ...
    \ << v); ++s) {\n      enumerate_all_bit(s, [&](int a) -> void {\n        enumerate_all_bit(nbd[a]\
    \ & mask & (~s), [&](int b) -> void {\n          dp[v * (s | 1 << b) + b] += dp[v\
    \ * s + a];\n        });\n        if (popcnt(s) >= 2 && nbd[a] >> v & 1) cyc[s\
    \ | 1 << v] += dp[v * s + a];\n      });\n    }\n  }\n  for (auto& x : cyc) x\
    \ /= T(2);\n  return cyc;\n}\n#line 8 \"test/3_yukicoder/2507.test.cpp\"\n\nvoid\
    \ solve() {\n  LL(N, M);\n  Graph<int, 0> G(N);\n  G.read_graph(M);\n\n  vc<u64>\
    \ dp = count_cycle<u64>(G);\n  FOR(v, N) dp[1 << v] += 1;\n  for (auto& e: G.edges)\
    \ {\n    int a = e.frm, b = e.to;\n    vc<u64> A(1 << N), B(1 << N);\n    FOR(s,\
    \ 1 << N) {\n      if (s >> a & 1) A[s] += dp[s];\n      if (s >> b & 1) B[s]\
