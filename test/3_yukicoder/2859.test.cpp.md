...
data:
  ...
  bundledCode: "#line 1 \"test/3_yukicoder/2859.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/2859\"\
    ...
    void Bob(bool t = 1) { Alice(!t); }\n#line 4 \"test/3_yukicoder/2859.test.cpp\"\
    ...
    \ [](int i) -> X { return MX::id(); });\n  }\n  void build(const vc<X>& v) {\n\
    \    build(len(v), [&](int i) -> X { return v[i]; });\n  }\n  template <typename\
    \ F>\n  void build(int m, F f) {\n    n = m, log = 1;\n    while ((1 << log) <\
    \ n) ++log;\n    size = 1 << log;\n    dat.assign(size << 1, MX::id());\n    FOR(i,\
    \ n) dat[size + i] = f(i);\n    FOR_R(i, 1, size) update(i);\n  }\n\n  X get(int\
    \ i) const { return dat[size + i]; }\n  vc<X> get_all() const { return {dat.begin()\
    ...
    \ /= 2;\n    }\n    return x;\n  }\n};\n#line 8 \"test/3_yukicoder/2859.test.cpp\"\
    \n\nvoid solve() {\n  LL(N, K);\n  VEC(ll, T, N);\n  VEC(ll, X, N);\n  VEC(ll,\
    \ C, N);\n\n  auto I = argsort(T);\n  T = rearrange(T, I);\n  X = rearrange(X,\
    \ I);\n  C = rearrange(C, I);\n\n  vi A(N), B(N);\n  FOR(i, N) A[i] = X[i] + K\
    \ * T[i];\n  FOR(i, N) B[i] = K * T[i] - X[i];\n\n  Wavelet_Matrix_2D_Range<ll,\
    \ 1, 1, SegTree<Monoid_Max<ll>>> seg(\n      N, [&](int i) -> tuple<ll, ll, ll>\
    \ { return {A[i], B[i], -infty<ll>}; });\n\n  ll ANS = 0;\n  FOR(i, N) {\n   \
    \ if (abs(X[i]) > K * T[i]) continue;\n    ll ans = 0;\n    chmax(ans, seg.prefix_prod(-infty<ll>,\
    \ A[i] + 1, B[i] + 1));\n    ans += C[i];\n    seg.set(i, ans);\n    chmax(ANS,\
    \ ans);\n  }\n  print(ANS);\n}\n\nsigned main() {\n  int T = 1;\n  // INT(T);\n\
    \  FOR(T) solve();\n  return 0;\n}\n"
  code: "#define PROBLEM \"https://yukicoder.me/problems/no/2859\"\n#include \"my_template.hpp\"\
    ...
    \  FOR(i, N) B[i] = K * T[i] - X[i];\n\n  Wavelet_Matrix_2D_Range<ll, 1, 1, SegTree<Monoid_Max<ll>>>\
    \ seg(\n      N, [&](int i) -> tuple<ll, ll, ll> { return {A[i], B[i], -infty<ll>};\
    \ });\n\n  ll ANS = 0;\n  FOR(i, N) {\n    if (abs(X[i]) > K * T[i]) continue;\n\
    \    ll ans = 0;\n    chmax(ans, seg.prefix_prod(-infty<ll>, A[i] + 1, B[i] +\
    \ 1));\n    ans += C[i];\n    seg.set(i, ans);\n    chmax(ANS, ans);\n  }\n  print(ANS);\n\
    }\n\nsigned main() {\n  int T = 1;\n  // INT(T);\n  FOR(T) solve();\n  return\
    \ 0;\n}\n"
  dependsOn:
