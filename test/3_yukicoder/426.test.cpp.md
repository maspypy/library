...
data:
  ...
  bundledCode: "#line 1 \"test/3_yukicoder/426.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/426\"\
    ...
    \  return C;\n}\n#line 7 \"test/3_yukicoder/426.test.cpp\"\n\nusing mint = modint107;\n\
    using mat = vc<vc<mint>>;\n\nstruct Mono {\n  using value_type = tuple<mat, mat,\
    \ mat>;\n  using X = value_type;\n  static X op(X x, X y) {\n    auto& [A, B,\
    \ C] = x;\n    auto& [D, E, F] = y;\n    auto P = matrix_mul(D, A);\n    auto\
    \ Q = matrix_mul(matrix_mul(C, E), A);\n    FOR(i, 2) FOR(j, 3) Q[i][j] += B[i][j];\n\
  ...
  verifiedWith: []
documentation_of: test/3_yukicoder/426.test.cpp
layout: document
redirect_from:
- /verify/test/3_yukicoder/426.test.cpp
- /verify/test/3_yukicoder/426.test.cpp.html
