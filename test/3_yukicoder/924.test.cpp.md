...
data:
  ...
  bundledCode: "#line 1 \"test/3_yukicoder/924.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/924\"\
    ...
    \n\nvoid solve() {\n  LL(N, Q);\n  VEC(ll, A, N);\n  Wavelet_Matrix<int, true,\
    \ Static_Range_Product_Group<Monoid_Add<ll>>> WM(\n      N, [&](int i) -> pair<int,\
    \ ll> { return {A[i], A[i]}; });\n  FOR(Q) {\n    LL(L, R);\n    --L;\n    int\
    \ n = (R - L);\n    ll k = (n - 1) / 2;\n    ll med = WM.kth(L, R, k);\n    ll\
    \ sum_lo = WM.prefix_prod(L, R, med);\n    auto sum_hi = WM.prod_all(L, R) - sum_lo;\n\
    \    ll ANS = 0;\n    ANS += med * k - sum_lo;\n    ANS += sum_hi - med * (R -\
    \ L - k);\n    print(ANS);\n  }\n}\n\nsigned main() {\n  solve();\n  return 0;\n\
    }\n"
  code: "#define PROBLEM \"https://yukicoder.me/problems/no/924\"\n#include \"my_template.hpp\"\
    ...
    \  VEC(ll, A, N);\n  Wavelet_Matrix<int, true, Static_Range_Product_Group<Monoid_Add<ll>>>\
    \ WM(\n      N, [&](int i) -> pair<int, ll> { return {A[i], A[i]}; });\n  FOR(Q)\
    \ {\n    LL(L, R);\n    --L;\n    int n = (R - L);\n    ll k = (n - 1) / 2;\n\
    \    ll med = WM.kth(L, R, k);\n    ll sum_lo = WM.prefix_prod(L, R, med);\n \
    \   auto sum_hi = WM.prod_all(L, R) - sum_lo;\n    ll ANS = 0;\n    ANS += med\
