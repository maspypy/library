...
data:
  ...
  bundledCode: "#line 1 \"test/2_library_checker/data_structure/predecessor_problem_4.test.cpp\"\
    ...
    \  return 64 * idx + topbit(dat[idx]);\n  }\n};\n#line 7 \"test/2_library_checker/data_structure/predecessor_problem_4.test.cpp\"\
    \n\nvoid solve() {\n  INT(N, Q);\n  STR(T);\n\n  FenwickTree_01 bit(N, [&](int\
    \ i) -> int { return T[i] - '0'; });\n  FOR(Q) {\n    INT(t, k);\n    if (t ==\
    \ 0) {\n      if (bit.sum(k, k + 1) == 0) bit.add(k, 1);\n    }\n    if (t ==\
    \ 1) {\n      if (bit.sum(k, k + 1)) bit.add(k, -1);\n    }\n    if (t == 2) {\n\
    \      int ans = bit.sum(k, k + 1);\n      print(ans);\n    }\n    if (t == 3)\
    \ {\n      int n = bit.sum(0, k);\n      int ans = (n == bit.sum_all() ? -1 :\
    \ bit.kth(n));\n      print(ans);\n    }\n    if (t == 4) {\n      int n = bit.sum(0,\
...
layout: document
redirect_from:
- /verify/test/2_library_checker/data_structure/predecessor_problem_4.test.cpp
- /verify/test/2_library_checker/data_structure/predecessor_problem_4.test.cpp.html
