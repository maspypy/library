...
data:
  ...
  bundledCode: "#line 1 \"test/5_atcoder/abc281g.test.cpp\"\n#define PROBLEM \"https://atcoder.jp/contests/abc281/tasks/abc281_g\"\
    ...
    \ yes(!t); }\r\n#line 4 \"test/5_atcoder/abc281g.test.cpp\"\n\n#line 2 \"mod/dynamic_modint.hpp\"\
    ...
    \ {\n    for (auto&& [j, x]: dat) {\n      if (i >= j) f[i] += x * f[i - j];\n\
    \    }\n  }\n  return f;\n}\n#line 9 \"test/5_atcoder/abc281g.test.cpp\"\n\nusing\
    \ mint = dmint;\n\nvoid solve() {\n  LL(N, mod);\n  mint::set_mod(mod);\n  auto\
    \ POW = powertable_1<mint>(mint(2), N * N + 10);\n\n  // \u4E00\u5FDC o(N^3) \u3092\
    \u5B9F\u88C5\u3059\u308B\u304B\n\n  vv(mint, dp, N, N);\n  FOR(i, N) dp[i][i]\
    \ += POW[i * (i - 1) / 2];\n\n  FOR(n, N) {\n    vc<mint> f = dp[n];\n    f.resize(n\
    \ + 1);\n    vc<mint> g(N - n);\n    /*\n    FOR(i, n + 1) {\n      FOR(j, 1,\
    \ N - n) { g[j] += f[i] * (POW[i] - mint(1)).pow(j); }\n    }\n    */\n    //\
    \ \u7D50\u5C40\u3001sum f[i]/1-ax \u306E\u5F62\n    vc<mint> point, wt;\n    FOR(i,\
  ...
  verifiedWith: []
documentation_of: test/5_atcoder/abc281g.test.cpp
layout: document
redirect_from:
- /verify/test/5_atcoder/abc281g.test.cpp
- /verify/test/5_atcoder/abc281g.test.cpp.html
