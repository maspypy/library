...
data:
  ...
  attributes:
    ...
    PROBLEM: https://yukicoder.me/problems/no/1434
    links:
    - https://yukicoder.me/problems/no/1434
  bundledCode: "#line 1 \"test/yukicoder/1434.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/1434\"\
    ...
    \    return A;\n  }\n\n  void reset() {\n    n_comp = n;\n    size.assign(n, 1);\n\
    \    std::iota(par.begin(), par.end(), 0);\n  }\n};\n#line 1 \"other/transpose.hpp\"\
    \n// vvc<T> \u3068 vc<string> \u3092\u60F3\u5B9A\ntemplate <typename VC>\nvc<VC>\
    \ transpose(const vc<VC>& A) {\n  int H = len(A), W = len(A[0]);\n  vc<VC> B(W,\
    \ VC(H, A[0][0]));\n  FOR(x, H) FOR(y, W) B[y][x] = A[x][y];\n  return B;\n}\n\
    #line 6 \"test/yukicoder/1434.test.cpp\"\n\nvoid solve() {\n  LL(H1, W1);\n  ll\
    ...
    \ y) -> int { return W * x + y; };\n    UnionFind uf(H * W);\n    auto ope = [&](int\
    \ x, int y, int x1, int y1) -> void {\n      int a = idx(x, y), b = idx(x1, y1);\n\
    \      assert(uf[a] != uf[b]);\n      uf.merge(a, b);\n      FOR(i, 2 * x, 2 *\
    \ x1 + 1) FOR(j, 2 * y, 2 * y1 + 1) ANS[i][j] = '.';\n    };\n\n    FOR(x, H)\
    \ FOR(y, W) {\n      if (x + 1 < H && abs(A[x][y] - A[x + 1][y]) == 1) ope(x,\
    \ y, x + 1, y);\n      if (y + 1 < W && abs(A[x][y] - A[x][y + 1]) == 1) ope(x,\
    \ y, x, y + 1);\n    }\n    FOR(x, H) FOR(y, W) {\n      if (x + 1 < H && uf[idx(x,\
    \ y)] != uf[idx(x + 1, y)]) ope(x, y, x + 1, y);\n      if (y + 1 < W && uf[idx(x,\
    \ y)] != uf[idx(x, y + 1)]) ope(x, y, x, y + 1);\n    }\n\n    if (sw) { ANS =\
    ...
    \n  ll T = 1;\n  // LL(T);\n  FOR(T) solve();\n\n  return 0;\n}\n"
  code: "#define PROBLEM \"https://yukicoder.me/problems/no/1434\"\n#include \"my_template.hpp\"\
    ...
    \ = '.';\n    };\n\n    FOR(x, H) FOR(y, W) {\n      if (x + 1 < H && abs(A[x][y]\
    \ - A[x + 1][y]) == 1) ope(x, y, x + 1, y);\n      if (y + 1 < W && abs(A[x][y]\
    \ - A[x][y + 1]) == 1) ope(x, y, x, y + 1);\n    }\n    FOR(x, H) FOR(y, W) {\n\
  ...
  verifiedWith: []
documentation_of: test/yukicoder/1434.test.cpp
layout: document
redirect_from:
- /verify/test/yukicoder/1434.test.cpp
- /verify/test/yukicoder/1434.test.cpp.html
