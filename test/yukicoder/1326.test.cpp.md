...
data:
  ...
  bundledCode: "#line 1 \"test/yukicoder/1326.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/1326\"\
    ...
    \    tree.debug();\r\n    bit.debug();\r\n    bit_subtree.debug();\r\n  }\r\n\
    };\r\n#line 7 \"test/yukicoder/1326.test.cpp\"\n\nvoid solve() {\n  LL(N, M);\n\
    \  Graph<int> G(N);\n  G.read_graph(M);\n  auto BCT = block_cut<decltype(G)>(G);\n\
    \  TREE<decltype(BCT)> tree(BCT);\n\n  vc<int> dat(tree.N);\n  FOR(v, N) if (BCT.deg(v)\
    \ > 1) dat[v] = 1;\n  TreeAbelGroup<decltype(tree), Group_Add<int>, 0, 1, 0> TA(tree,\
    \ dat);\n\n  LL(Q);\n  FOR(Q) {\n    LL(x, y);\n    --x, --y;\n    if (x == y)\
