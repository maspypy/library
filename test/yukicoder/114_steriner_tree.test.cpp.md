...
data:
  ...
  bundledCode: "#line 1 \"test/yukicoder/114_steriner_tree.test.cpp\"\n#define PROBLEM\
    ...
    */\ntemplate <typename T, typename Graph>\nT steiner_tree(Graph& G, vc<int> terminal,\
    \ T INF) {\n  int k = len(terminal);\n  if (k <= 1) return 0;\n  int n = G.N;\n\
    \  vv(T, DP, 1 << k, n, INF);\n  FOR(i, k) { DP[1 << i][terminal[i]] = 0; }\n\
    \  FOR3(s, 1, 1 << k) {\n    auto& dp = DP[s];\n    FOR_subset(t, s) { FOR(v,\
    \ n) chmin(dp[v], DP[t][v] + DP[s ^ t][v]); }\n    pqg<pair<T, int>> que;\n  \
...
layout: document
redirect_from:
- /verify/test/yukicoder/114_steriner_tree.test.cpp
- /verify/test/yukicoder/114_steriner_tree.test.cpp.html
