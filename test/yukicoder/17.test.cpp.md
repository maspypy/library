...
data:
  ...
  bundledCode: "#line 1 \"test/yukicoder/17.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/17\"\
    ...
    \u304C\u3042\u3063\u3066\u3082\u8CA0\u9589\u8DEF\u304C\u306A\u3051\u308C\u3070\
    \u6B63\u3057\u304F\u52D5\u4F5C\u3059\u308B\u3002\n// \u8CA0\u9589\u8DEF\u304C\u3042\
    \u308B\u304B\u3069\u3046\u304B\u306F\u3001dist[v][v] < 0 \u3068\u306A\u308B v\
    \ \u304C\u3042\u308B\u304B\u3069\u3046\u304B\u3067\u5224\u5B9A\u3002\ntemplate\
    \ <typename T, typename GT>\nvc<vc<T>> warshall_floyd(GT& G) {\n  ll N = G.N;\n\
    \  vv(T, dist, N, N, infty<T>);\n  FOR(v, N) {\n    dist[v][v] = 0;\n    for (auto&&\
    \ e: G[v]) chmin(dist[v][e.to], e.cost);\n  }\n  FOR(k, N) FOR(i, N) {\n    if\
    \ (dist[i][k] == infty<T>) continue;\n    FOR(j, N) {\n      if (dist[k][j] ==\
    \ infty<T>) continue;\n      chmin(dist[i][j], dist[i][k] + dist[k][j]);\n   \
    \ }\n  }\n  return dist;\n}\n#line 5 \"test/yukicoder/17.test.cpp\"\n\nvoid solve()\
    \ {\n  LL(N);\n  VEC(ll, COST, N);\n  Graph<ll> G(N);\n  LL(M);\n  G.read_graph(M,\
    \ 1, 0);\n\n  auto dist = warshall_floyd<ll>(G);\n\n  ll ANS = infty<ll>;\n  FOR3(i,\
    \ 1, N - 1) FOR3(j, 1, N - 1) if (i != j) {\n    chmin(ANS, dist[0][i] + dist[i][j]\
    \ + dist[j][N - 1] + COST[i] + COST[j]);\n  }\n  print(ANS);\n}\n\nsigned main()\
  ...
  verifiedWith: []
documentation_of: test/yukicoder/17.test.cpp
layout: document
redirect_from:
- /verify/test/yukicoder/17.test.cpp
- /verify/test/yukicoder/17.test.cpp.html
