...
data:
  ...
  bundledCode: "#line 1 \"test/yukicoder/705.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/705\"\
    ...
    \ state;\n    std::unique_ptr<reduce_col> rec;\n\n    reduce_row(int n_) : n(n_),\
    \ f(), cur_row(0), state(0), rec() {\n      const int m = n / 2;\n      if (m\
    \ != 0) { rec = std::make_unique<reduce_col>(m); }\n    }\n\n    void set_f(std::function<T(int,\
    \ int)> f_) {\n      f = f_;\n      if (rec) {\n        rec->set_f([&](int i,\
    \ int j) -> T { return f(2 * i + 1, j); });\n      }\n    }\n\n    int get_argmin()\
