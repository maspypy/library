...
data:
  ...
  bundledCode: "#line 1 \"test/1_mytest/lattice_point_sum_polynomial.test.cpp\"\n\
    ...
    \ allow_180=true \u3067\u540C\u4E00\u5EA7\u6A19\u70B9\u304C\u3042\u308B\u3068\u3053\
    \u308F\u308C\u308B\n// full \u306A\u3089 I[0] \u304C sorted \u3067 min \u306B\u306A\
    ...
    \ 4);\n      ll b = RNG(-3, 4);\n      if (a == 0 && b == 0) continue;\n     \
    \ ll c = RNG(-30, 31);\n      LINE.eb(a, b, c);\n      break;\n    }\n  }\n\n\
    \  array<array<mint, K2 + 1>, K1 + 1> god{};\n  auto add_point = [&](ll x, ll\
    \ y) -> void {\n    FOR(i, K1 + 1) FOR(j, K2 + 1) { god[i][j] += mint(x).pow(i)\
    \ * mint(y).pow(j); }\n  };\n\n  ll K = 200;\n  FOR(x, -2 * K, 2 * K + 1) {\n\
    ...
    \ = lattice_point_sum_polynomial<mint, K1, K2>(LINE);\n  assert(god == ANS);\n\
    }\n\nvoid solve() {\n  int a, b;\n  cin >> a >> b;\n  cout << a + b << \"\\n\"\
    ;\n}\n\nsigned main() {\n  FOR(1000) {\n    test<0, 0>();\n    test<0, 1>();\n\
    \    test<0, 2>();\n    test<0, 3>();\n    test<1, 0>();\n    test<1, 1>();\n\
    \    test<1, 2>();\n    test<1, 3>();\n    test<2, 0>();\n    test<2, 1>();\n\
