...
data:
  ...
  bundledCode: "#line 1 \"test/1_mytest/mo_on_tree.test.cpp\"\n#define PROBLEM \"\
    ...
    \      } else {\n        AFF f = Mono::id();\n        auto get = [&](int a, int\
    \ b) -> int { return tree.v_to_e((tree.parent[a] == b ? a : b)); };\n        auto\
    \ init = [&]() -> void {};\n        auto add_l = [&](int a, int b) -> void { f\
    \ = Mono::op(dat[get(a, b)], f); };\n        auto rm_l = [&](int a, int b) ->\
    \ void { f = Mono::op(Mono::inverse(dat[get(a, b)]), f); };\n        auto add_r\
    \ = [&](int a, int b) -> void { f = Mono::op(f, dat[get(a, b)]); };\n        auto\
    \ rm_r = [&](int a, int b) -> void { f = Mono::op(f, Mono::inverse(dat[get(a,\
    \ b)])); };\n        auto ans = [&](int q) -> void { assert(f == TM.prod_path(query[q].fi,\
