...
data:
  ...
  code: "#define PROBLEM \"https://judge.yosupo.jp/problem/aplusb\"\n\n#include \"\
    ...
    \ {\n  int N = len(f);\n  vc<mint> g(N);\n  if (!transpose) {\n    FOR(i, N) FOR(j,\
    \ N) if (i >= j) g[i] += C<mint>(i, j) * f[j];\n  }\n  if (transpose) {\n    FOR(i,\
    \ N) FOR(j, N) if (i <= j) g[i] += C<mint>(j, i) * f[j];\n  }\n  return g;\n}\n\
    \nvoid test() {\n  FOR(t, 2) {\n    FOR(N, 1, 100) {\n      vc<mint> f = gen(N);\n\
    \      vc<mint> g1 = pascal<mint>(f, t, 0);\n      vc<mint> g2 = pascal_naive(f,\
