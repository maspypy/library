...
data:
  ...
  code: "\npair<vvc<int>, vvc<int>> rsk_correspondence_prefix(vc<int> A, int nrow\
    ...
    \      } else {\n        swap(X[row][col], a);\n      }\n    }\n  }\n  while (len(X)\
    \ && X.back().empty()) POP(X);\n  return {X, Y};\n}\n\n// \u6307\u6570\u63A2\u7D22\
    \u3068\u8EE2\u7F6E\u306E\u5229\u7528\u306B\u3088\u308A worst(N^1.5) \u306B\u306A\
    \u3063\u3066\u3044\u308B.\n// \u30E9\u30F3\u30C0\u30E0\u30B1\u30FC\u30B9\u3060\
    \u3068\u666E\u901A\u306B\u3084\u308B\u65B9\u304C\u901F\u3044.\npair<vvc<int>,\
    \ vvc<int>> rsk_correspondence(vc<int> A) {\n  int N = len(A);\n  if (N == 0)\
    \ return {};\n  int M = 1;\n  while (M * M < N) ++M;\n  auto [X1, Y1] = rsk_correspondence_prefix(A,\
    \ M);\n  reverse(all(A));\n  int m = len(X1);\n  auto [X2, Y2] = rsk_correspondence_prefix(A,\
    \ m);\n  reverse(all(A));\n  for (auto& x : A) x = N - 1 - x;\n  auto [X3, Y3]\
