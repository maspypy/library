...
void centroid_decomposition_2_dfs(GT& G, vc<int> vs, vc<int> real, F f) {
  ...
  FOR(v, N) {
    int i = ord[v];
    V[i] = vs[v], rea[i] = real[v];
    (i <= n1 ? V1[i] : V2[i - n1]) = vs[v];
    (i <= n1 ? rea1[i] : rea2[i - n1]) = real[v];
  }
  for (auto& e: G.edges) {
    int a = ord[e.frm], b = ord[e.to];
    if (a > b) swap(a, b);
  ...
  if (real[c]) {
    color.assign(N, -1);
    color[0] = 0;
    FOR(i, 1, N) color[i] = rea[i] ? 1 : -1;
    f(H, V, color);
    rea[0] = rea1[0] = rea2[0] = 0;
  }
  color.assign(N, -1);
  FOR(i, 1, N) if (rea[i]) color[i] = (i <= n1 ? 0 : 1);
  f(H, V, color);
  centroid_decomposition_2_dfs(H1, V1, rea1, f);
...
template <typename GT, typename F>
void centroid_decomposition_2(GT& G, F f) {
  if (G.N == 1) return;
  vc<int> real(G.N, 1);
  if constexpr (GT::is_directed) {
    vc<int> V = toposort(G);
