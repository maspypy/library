...
struct StaticTreeMonoid {
  ...
  StaticTreeMonoid(HLD &hld) : hld(hld), N(hld.N), seg(hld.N) {
    if (!Monoid::commute) seg_r = DisjointSparse<RevMonoid>(hld.N);
  }

  StaticTreeMonoid(HLD &hld, vc<X> &dat) : hld(hld), N(hld.N) {
